...
public class ApiResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<Book> bookByCategoryName(@PathParam("category-name") String categoryName,
                                         @Context HttpServletRequest httpRequest) {
        try {
            Category category = categoryDao.findByName(categoryName);
            if (category == null) {
            ...
            throw new ApiException(String.format("Book lookup by category name %s failed", categoryName), e);
        }
    }
